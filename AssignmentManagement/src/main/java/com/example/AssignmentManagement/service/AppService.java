...
public class AppService {
    ...
    public Assignments findAssignmentByTitle(String name) {
        Optional<Assignments> assignment = assignmentRepository.findByTitle(name);
        if (assignment.isPresent()) {
            return assignment.get();
        } else {
            throw new EntityNotFoundException("Assignment not found with name: " + name);
        }
    }
